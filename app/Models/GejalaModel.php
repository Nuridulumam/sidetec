...
class GejalaModel extends Model
{
    ...
    protected $beforeInsert = ['generateUuid', 'calculateDiagnosis'];
    ...
    protected function calculateDiagnosis(array $data)
    {
        ...
        // Fetch patient age from Pasien master table
        if (!empty($symptomsData['pasien_id'])) {
            $pasienModel = new \App\Models\PasienModel();
            $pasien = $pasienModel->find($symptomsData['pasien_id']);
            if ($pasien) {
                $symptomsData['usia'] = (int) $pasien['usia'];
            }
        }
